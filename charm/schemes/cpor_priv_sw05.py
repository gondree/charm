...
from charm.toolbox.POR import PORbase from charm.toolbox.integergroup import * 
...
class CPORpriv (PORbase):
	def __init__(self, common_input = None):
		...
		self.lambda = 80
		self.sector_size = (self.lambda - 1) / 8
		self.g = IntegerGroup()
	...
	def tag (self, filename, pk, sk):
		...
		# the number of sectors in a block
 		num_sectors = self.block_size / self.sector_size
		...
		num_blocks = Mprime / block_size 
		if (Mprime % self.block_size is not 0):
			num_blocks += 1 

		m = [[] for i in range(num_blocks)]
		...
		with open(filename, "rb") as f:
			...
			while block:
				...
				for j in range(num_sectors):
					...
					m[i].append(sectors[jstart:jend])
				block = f.read(self.block_size)
				i = i + 1
		#
		# make the tags:
		...
		kprf = bytes(int2bytes(kbits))
		# a random PRF key
		PRF = cryptobase.selectPRF(AES, (kprf, MODE_ECB))
		alpha = [self.g.random() for i in range(num_sectors)]
		# a list of random numbers from Zp, |alpha| = num_sectors
		...
		#
        sigmas =[]
		# for each block, a sigma is generated with a function using PRFkey on each block, adding the product of all the alphas, and the sectors
        for i in range (num_blocks):
			am = [alpha[j] * struct.unpack('>q', m[i][j])[0] for j in range (len(m[i]))]
			s = PRF(i) + sum(am) 
			sigmas.append(s) 
